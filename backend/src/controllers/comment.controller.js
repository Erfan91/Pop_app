...
const editComment = async (req, res, next) => {
    const { text } = req.body
    try {
        await Comment.findByIdAndUpdate(req.params.id, { text }, { new: true })
            .exec()
            .then(result => {
                if (!result) {
                    res.status(400).json({ message: "comment id incorrect or not registred", state: false })
                }
                res.status(200).json({ message: "comment edited successfuly", state: true });
            })
    } catch (error) {
        res.status(500).json({ message: "Internal server error", error: error.message })
    }
...
const deleteComment = async (req, res, next) => {
    try {
        ...
        await Post.findByIdAndUpdate(postId, { $pull: { comments: req.params.id } }, { new: true })
            ...
            .then(resp => {
                if (!resp) {
                    res.json({ message: "incorrect post id comment not found " })
                }
                if (resp) {
                    Comment.findByIdAndDelete(req.params.id)
                        ...
                        .then(result => {
                            if (!result) {
                                res.status(400).json({ message: "comment id incorrect or not registred", state: true })
                            }
                            res.status(200).json({ message: "comment deleted successfuly", state: false })
                        })
                }
